...
public class HiloC extends Thread{
    ...
    public HiloC(Socket socket, int id) {
        ...
        try {
            ...
            if(parts[0].equals("006")){
                System.out.println("006-Recibido datos de busqueda de archivo seleccionado.");
                DataInputStream input;
                BufferedInputStream bis;
                BufferedOutputStream bos;
                int in;
                ...
                try
                {
                    String path = control.buscarArchivoSeleccionado(parts[1], parts[2], parts[3]);
                    File file = new File(path);
                    System.out.println("006-Encontrado el archivo seleccionado.");
                    bis = new BufferedInputStream(new FileInputStream(file));                  
                    bos = new BufferedOutputStream(socket.getOutputStream());
                    DataOutputStream dos = new DataOutputStream(socket.getOutputStream());
                    dos.writeUTF(file.getName());
                    System.out.println("006-Enviado el archivo seleccionado.");
                    //Enviamos el fichero
                    byteArray = new byte[8192];
                    while ((in = bis.read(byteArray)) != -1){
                        bos.write(byteArray,0,in);
